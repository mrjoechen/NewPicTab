...
import { clearAllNewPicTabData, type DataClearResult } from '../dataClear';
...
export interface AboutPanelProps {
  ...
  onCleared: (settings: NewPicTabSettings) => void;
  modalBackground?: () => HTMLElement | null;
}

export function AboutPanel({ version = runtimeVersion(), repositoryUrl = PROJECT_REPOSITORY_URL, clearData = clearAllNewPicTabData, onCleared, modalBackground }: AboutPanelProps) {
  ...
  return <section ref={rootRef} className="settings-section about-panel" aria-labelledby="about-title">
    <header className="settings-section__header"><p className="settings-eyebrow">NewPicTab {version}</p><h2 id="about-title">关于与隐私</h2><p>一张背景，一点时间，其余保持安静。</p></header>

    <div className="about-block"><h3>隐私</h3><p>NewPicTab 不包含统计、遥测或跟踪，也没有 NewPicTab 服务器；持久化配置不会上传到 NewPicTab 基础设施。</p><p>仅在启用对应功能时，NewPicTab 才会直接请求你选择的第三方：WebDAV 会把凭据发送给 WebDAV 服务；JSON API 会把配置的请求头发送给 API endpoint，并从你授权的图片主机或 CDN 下载图片；在线图片 URL 会直接请求相应图片主机；TMDB 会把 API 凭据发送给 TMDB API，并从 TMDB CDN 下载图片；天气会把城市或坐标发送给 Open-Meteo；主动使用当前位置时，坐标还会发送给 BigDataCloud 以识别城市名称；搜索引擎图标随扩展内置；只有提交搜索后才会把查询交给所选搜索引擎；快捷网址则进行普通网页导航。</p><p>图片源凭据保存在 Chrome 本地存储；这能避免浏览器同步，但无法防止可访问你已解锁浏览器配置文件的人读取。WebDAV 推荐使用应用专用密码。</p><p>已授予的站点访问权限可能继续保留，直到你在 Chrome 扩展设置中移除；清除数据不会擅自撤销权限。</p></div>

    <div className="about-block"><h3>源码与许可</h3><p>NewPicTab 是采用 MIT License 发布的开源软件，可自由使用、修改和分发，包括商业用途；分发时须保留版权与许可声明。</p><div className="about-links"><a href={licenseUrl} target="_blank" rel="noopener noreferrer">查看 MIT License</a>{repository ? <a href={repository} target="_blank" rel="noopener noreferrer">源码仓库</a> : <span>仓库地址尚未配置。</span>}<ExternalLink href={PROJECT_DONATION_URL} className="about-link--with-icon"><img className="about-link__logo" src={runtimeAssetUrl('assets/ko-fi-logomark.webp')} alt="" />支持作者</ExternalLink></div></div>

    <div className="about-block"><h3>TMDB</h3><img className="tmdb-attribution__logo" src={runtimeAssetUrl('assets/tmdb-blue-short.svg')} alt="TMDB" /><p>{PROVIDERS.tmdb.attribution}</p><p>TMDB 内容与商标归其各自权利人所有；NewPicTab 未内置 API 凭据。</p><div className="about-links"><ExternalLink href={PROVIDERS.tmdb.applyUrl!}>申请 TMDB API 凭据</ExternalLink><ExternalLink href={PROVIDERS.tmdb.guideUrl}>TMDB 官方指南</ExternalLink><ExternalLink href={PROVIDERS.tmdb.attributionUrl!}>TMDB 标识与归因规范</ExternalLink></div></div>

    <div className="about-block about-danger"><h3>清除数据</h3><p>移除 NewPicTab 在此浏览器配置文件中的设置、凭据、图片与运行记录。</p><button ref={clearTriggerRef} type="button" className="button button--danger button--with-icon" aria-label="清除所有 NewPicTab 数据" title="清除所有 NewPicTab 数据" onClick={() => setConfirming(true)}><Icon name="trash" /><span>清除</span></button></div>
...
function ClearDataConfirmation({ background, restore, clearData, onCancel, onCleared }: { background: HTMLElement | null; restore: HTMLButtonElement | null; clearData: () => Promise<DataClearResult>; onCancel: () => void; onCleared: (settings: NewPicTabSettings) => void }) {
  ...
  return <div className="confirm-backdrop"><section className="confirm-dialog confirm-dialog--wide" role="alertdialog" aria-modal="true" aria-labelledby="clear-data-title" aria-describedby="clear-data-description">
    <h3 id="clear-data-title">清除所有 NewPicTab 数据</h3>
