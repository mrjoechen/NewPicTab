...
export function AboutPanel({ version = runtimeVersion(), repositoryUrl = PROJECT_REPOSITORY_URL, clearData = clearAllPicTabData, onCleared, modalBackground }: AboutPanelProps) {
  ...
  return <section ref={rootRef} className="settings-section about-panel" aria-labelledby="about-title">
    ...
    <div className="about-block"><h3>源码与许可</h3><p>PicTab 是采用 MIT License 发布的开源软件，可自由使用、修改和分发，包括商业用途；分发时须保留版权与许可声明。</p><div className="about-links"><a href={licenseUrl} target="_blank" rel="noopener noreferrer">查看 MIT License</a>{repository ? <a href={repository} target="_blank" rel="noopener noreferrer">源码仓库</a> : <span>仓库地址尚未配置。</span>}<ExternalLink href={PROJECT_DONATION_URL} className="about-link--with-icon"><img className="about-link__logo" src={runtimeAssetUrl('assets/ko-fi-logomark.webp')} alt="" />支持作者</ExternalLink></div></div>
