...
const ENGLISH_UI_TEXT: Record<string, string> = {
  ...
  '搜索': 'Search', '搜索词会直接交给所选引擎，PicTab 不会记录。': 'Search terms go directly to the selected engine. PicTab does not record them.', '显示搜索': 'Show search', '搜索引擎': 'Search engine', '自定义': 'Custom', '搜索模板': 'Search template', '保存搜索模板': 'Save search template',
  ...
  '天气': 'Weather', '默认手动选择城市，也可主动使用浏览器定位。': 'Choose a city manually, or use browser location when requested.', '显示天气': 'Show weather', '轻微天气动效': 'Subtle weather animation', '当前显示的是缓存天气，网络恢复后会自动刷新。': 'Cached weather is shown and will refresh when the network returns.', '搜索城市': 'Search cities', '城市搜索结果': 'City search results', '定位仅在点击此按钮后读取一次，用来向 Open-Meteo 查询当地天气；PicTab 不会持续追踪位置。': 'Location is read once only after you click this button and is used to query local weather from Open-Meteo. PicTab does not continuously track your location.', '当前位置': 'Current location', '使用当前位置': 'Use current location', '定位': 'Locate', '请输入至少两个字符。': 'Enter at least two characters.', '未授予天气服务访问权限。': 'Weather service access was not granted.', '没有找到匹配的城市。': 'No matching cities found.', '暂时无法搜索城市。': 'Unable to search for cities right now.', '已选择城市。': 'City selected.', '未能读取位置，请改用城市。': 'Unable to read your location. Choose a city instead.', '已使用当前位置。': 'Using the current location.', '位置已保存，天气稍后自动刷新。': 'Location saved. Weather will refresh automatically.',
  '图库': 'Library', '图片源': 'Sources', '添加多个来源，随时切换当前展示的图库。': 'Add multiple sources and switch the displayed library at any time.', '还没有图片源': 'No image sources yet', '添加后，PicTab 会立即显示其中的图片。': 'After you add one, PicTab will display its images immediately.', '已停用': 'Disabled', '待检测': 'Not checked', '可用': 'Available', '检测中': 'Checking', '缓存可用': 'Cached', '不可用': 'Unavailable', '正在使用': 'In use', '图片数量待加载': 'Image count pending', '已启用': 'Enabled', '使用此源': 'Use this source', '使用': 'Use', '重命名': 'Rename', '命名': 'Rename', '配置': 'Configure', '测试': 'Test', '刷新': 'Refresh', '清除缓存': 'Clear cache', '缓存': 'Cache', '删除': 'Delete', '选择图片源类型': 'Choose image source type', '选择来源': 'Choose a source', '取消': 'Cancel', '本地图片': 'Local images', '从当前设备导入': 'Import from this device', '连接你的私有图库': 'Connect your private library', '在线图片 URL': 'Direct image URLs', '逐行添加 HTTPS 图片': 'Add HTTPS images one per row', '映射自定义接口字段': 'Map fields from a custom endpoint', '电影与电视背景图': 'Movie and TV backgrounds', '添加图片源': 'Add image source', '添加': 'Add',
  ...
  '关于与隐私': 'About and privacy', '一张背景，一点时间，其余保持安静。': 'One background, a little time, and everything else stays quiet.', '隐私': 'Privacy', 'PicTab 不包含统计、遥测或跟踪，也没有 PicTab 服务器；持久化配置不会上传到 PicTab 基础设施。': 'PicTab contains no analytics, telemetry, or tracking and operates no PicTab servers. Persisted configuration is not uploaded to PicTab infrastructure.', '仅在启用对应功能时，PicTab 才会直接请求你选择的第三方：WebDAV 会把凭据发送给 WebDAV 服务；JSON API 会把配置的请求头发送给 API endpoint，并从你授权的图片主机或 CDN 下载图片；在线图片 URL 会直接请求相应图片主机；TMDB 会把 API 凭据发送给 TMDB API，并从 TMDB CDN 下载图片；天气会把城市或坐标发送给 Open-Meteo；主动使用当前位置时，坐标还会发送给 BigDataCloud 以识别城市名称；搜索控件会从内置搜索服务加载图标，提交后才把查询交给所选搜索引擎；快捷网址则进行普通网页导航。': 'PicTab contacts the third parties you choose only when you enable the corresponding feature: WebDAV sends credentials to the WebDAV service; JSON API sends configured request headers to the API endpoint and downloads images from image hosts or CDNs you authorize; direct image URLs contact their respective image hosts; TMDB sends API credentials to the TMDB API and downloads images from the TMDB CDN; weather sends a city or coordinates to Open-Meteo; when you explicitly use your current location, coordinates are also sent to BigDataCloud to identify the city name; the search widget loads icons from built-in search services and sends a query to the selected search engine only after submission; shortcuts perform ordinary web navigation.', '图片源凭据保存在 Chrome 本地存储；这能避免浏览器同步，但无法防止可访问你已解锁浏览器配置文件的人读取。WebDAV 推荐使用应用专用密码。': 'Image-source credentials are stored in Chrome local storage. This prevents browser sync but cannot prevent someone with access to your unlocked browser profile from reading them. Use an app-specific password for WebDAV.', '已授予的站点访问权限可能继续保留，直到你在 Chrome 扩展设置中移除；清除数据不会擅自撤销权限。': 'Granted site access may remain until you remove it in Chrome extension settings. Clearing data does not automatically revoke permissions.', '源码与许可': 'Source and license', 'PicTab 是采用 MIT License 发布的开源软件，可自由使用、修改和分发，包括商业用途；分发时须保留版权与许可声明。': 'PicTab is open-source software released under the MIT License. You may use, modify, and distribute it, including for commercial purposes, provided that the copyright and license notices are retained.', '查看 MIT License': 'View MIT License', '源码仓库': 'Source repository', '仓库地址尚未配置。': 'The repository URL is not configured.', '申请 TMDB API 凭据': 'Apply for TMDB API credentials', 'TMDB 官方指南': 'Official TMDB guide', 'TMDB 标识与归因规范': 'TMDB logo and attribution guidelines', 'TMDB 内容与商标归其各自权利人所有；PicTab 未内置 API 凭据。': 'TMDB content and trademarks belong to their respective rights holders. PicTab does not include API credentials.', '清除数据': 'Clear data', '移除 PicTab 在此浏览器配置文件中的设置、凭据、图片与运行记录。': 'Remove PicTab settings, credentials, images, and runtime records from this browser profile.', '清除所有 PicTab 数据': 'Clear all PicTab data', '清除': 'Clear', '将从当前浏览器配置文件永久移除：': 'The following will be permanently removed from this browser profile:', '设置与凭据': 'Settings and credentials', '远程图片缓存与目录': 'Remote image cache and catalog', '天气缓存': 'Weather cache', '切换游标与清理日志': 'Rotation cursors and cleanup journals', '不会删除 WebDAV 或 TMDB 上的远端内容。此操作无法撤销。': 'Remote content on WebDAV or TMDB will not be deleted. This action cannot be undone.', '正在清除…': 'Clearing…', '确认清除': 'Confirm clear', '清除中': 'Clearing', '未知版本': 'Unknown version',
